...
- This file is an operating guide plus concise product contract.
- If any instruction conflicts with runtime behavior or DB constraints, follow `SYSTEM_SPEC.md` and current code.
- Read `SYSTEM_SPEC.md` selectively: determine task scope first, then read only the relevant sections (and nearby constraints), not the entire file.
